...
const Orderpages = () => {
  ...
  const getHistory = async (selectedOrderId = null) => {
    axios
      .get(
        `http://rsudsamrat.site:8990/api/bid-exchange/history/${
      ...
      .then((response) => {
        ...
        axios
          .post("http://rsudsamrat.site:8990/api/v1/notifikasi", {
            ...
            message: `${selectedOrder.id}, Pesanan ${selectedOrder.orderItems[0].productName} telah diterima.`,
          })
          .then((res) => {
  ...
  const handleAcceptOrder = (orderItemId) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setActionToastBody(
          `${selectedOrder.id},Your offer for the product ${selectedOrderItem.product.name} has been accepted.`
        );
        setShowActionToast(true);
        setTimeout(() => {
          setShowActionToast(false);
      ...
      .then((response) => {
        ...
        axios
          .post(`http://rsudsamrat.site:8990/api/v1/notifikasi`, {
            ...
            message: `${selectedOrder.id}, Penawaran Produk ${selectedOrderItem.product.name}`,
          })
          .catch((err) => console.log(err));
      })
      .catch((error) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        axios.post(`http://rsudsamrat.site:8990/api/v1/notifikasi`, {
          ...
          receiver: selectedOrderItem.product.vendor.name,
          receiverId: selectedOrderItem.product.vendor.id,
          message: `${selectedOrder.id}, Product ${selectedOrderItem.id} Accepted`,
        });
      })
      .catch((error) => {
        // Handle any error that occurred during the API call
        ...
        <div>
          ...
          {showActionToast && (
            <div className="toast toast-end z-[99999]">
              <div className="flex flex-col items-start justify-start gap-2 alert">
